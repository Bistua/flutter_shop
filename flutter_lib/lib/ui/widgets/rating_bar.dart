...
Path createStarPath(double radius, double radiusRatio, Path path) {
  ...
  path.lineTo((radius * Math.cos(radian / 2) * 2),
      (radius - radius * Math.sin(radian / 2)));
  path.lineTo((radius * Math.cos(radian / 2) + radiusIn * Math.cos(radian / 2)),
...
class RatingBar extends StatefulWidget {
  ...
  RatingBar(
      ...
      Color colorDark,
      Color colorLight})
      : colorDark = colorDark ?? new Color(0xFFDCDCDC),
        colorLight = colorLight ?? new Color(0xFFFFBE0D);
...
class _RatingBarState extends State<RatingBar> {
  ...
  Widget build(BuildContext context) {
    ...
    double interval = widget.interval;

    List<Widget> list = [];
    for (int i = 0; i < count; ++i) {
      list.add(buildItem(i, size, count));
      if (interval > 0) {
        list.add(Container(width: interval));
      }
    }
